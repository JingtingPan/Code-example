...
public:
	...
	bool operator==(const Stack<T>& b) const;	// operator==
	void print();		// print
	int search(T key);	// search
	int search(T key, int nth);
	int replace(Stack<T>& stack, T s, T d);
...
template<typename T>
int Stack<T>::search(T key, int nth)
{
	int count = 0;
	StackNode<T>* pNewTos = pTos;
	int pos = stackSize;
...
template<typename T>
int Stack<T>::replace(Stack<T>& stack, T s, T d)
{
	int count = 0;
	StackNode<T>* pNewTos = pTos;
	int pos = stackSize;
