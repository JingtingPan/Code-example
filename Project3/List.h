...
template<typename T>
void List<T>::set(int p, T item) // set node p¡¯s item to item 
{
	if (p < 0 || p >= listSize) {
		std::cout << "Unable to reach list position " << p << std::endl;
		exit(0);
...
template<typename T>
void List<T>::set_next(T item) // set pCurrentNode->item to item, move pCurrentNode to 
 // pCurrentNode->pNextNode 
{
	if (pCurrentNode != NULL)
	{
...
template<typename T>
void List<T>::set_prev(T item)// set pCurrentNode->item to item, move pCurrentNode to 
	 // pCurrentNode->p
{
	if (pCurrentNode != NULL) 
	{
